...
def check_packages(packname):
    utils = importr('utils')
    utils.chooseCRANmirror(ind=1)
    try:
        print(f'Checking for package {packname}')
        rpack = importr(packname)
    except:
        print(f'{packname} not installed, performing one time installation')
        biocmanager = importr('BiocManager')
        biocmanager.install(packname)
